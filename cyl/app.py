...
class ClockApp(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        if self.config is None:
            ...
            logger.info("loaded existing config (birthday=%s, target=%d)", self.config.birthday, self.config.target_age)
            self._update_time()
    ...
    def _request_birthday(self) -> None:
        ...
        if dialog.exec():
            ...
            logger.info("new config saved (birthday=%s, target=%d)", self.config.birthday, self.config.target_age)
            self._update_time()
        else:
